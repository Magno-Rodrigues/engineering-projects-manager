...
class TestCostCenterService:
    """Tests for CostCenterService."""

    def test_create_cost_center_success(self, app, db, fin_project):
        """Test creating a cost center successfully."""
        with app.app_context():
            ...
            assert cc.budget_allocation == Decimal('50000')
            db.session.delete(cc)
            db.session.commit()

    def test_create_cost_center_invalid_project(self, app, db):
        """Test creating a cost center for a non-existent project."""
        with app.app_context():
            cc, error = CostCenterService.create_cost_center(project_id=99999, name='Test')
            assert cc is None
            assert 'not found' in error.lower()

    def test_create_cost_center_missing_name(self, app, db, fin_project):
        """Test that missing name returns an error."""
        with app.app_context():
            cc, error = CostCenterService.create_cost_center(project_id=fin_project, name='')
            assert cc is None
            assert error is not None
    ...
    def test_get_project_cost_centers(self, app, db, fin_project):
        ...
        with app.app_context():
            ...
            assert 'CC1' in names
            assert 'CC2' in names
            db.session.delete(cc1)
            db.session.delete(cc2)
            db.session.commit()

    def test_delete_cost_center_success(self, app, db, fin_project):
        """Test deleting a cost center."""
        with app.app_context():
            cc, _ = CostCenterService.create_cost_center(project_id=fin_project, name='ToDelete')
            cc_id = cc.id
