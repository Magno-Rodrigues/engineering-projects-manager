...
class TestCostCenterService:
    ...
    def test_create_cost_center_success(self, app, db, fin_project):
        ...
        with app.app_context():
            ...
            success, assoc_error = CostCenterService.associate_with_project(cc.id, fin_project)
            assert success is True
            assert assoc_error is None
    ...
    def test_associate_duplicate(self, app, db, fin_project):
        ...
        with app.app_context():
            ...
            assert success is False
            assert 'already associated' in error.lower()
            db.session.delete(cc)
            db.session.commit()
    ...
    def test_dissociate_from_project(self, app, db, fin_project):
        ...
        with app.app_context():
            ...
            assert not any(c.id == cc.id for c in ccs)
            db.session.delete(cc)
            db.session.commit()
