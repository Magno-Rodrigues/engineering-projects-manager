...
def cost_centers(project_id: int):
    ...
    if request.method == 'POST':
        ...
        if action == 'create':
            # Create a NEW cost center and associate with project
            ...
            if cc:
                flash('Centro de custo criado e associado com sucesso.', 'success')
            else:
                flash(error, 'error')
...
def associate_cost_center(project_id: int, cc_id: int):
    """Associate an existing cost center with a project."""
    project = _get_project_or_abort(project_id)
    if project is None:
        return redirect(url_for('projects.index'))

    success, error = CostCenterService.associate_cost_center_to_project(
        project_id=project_id,
        cost_center_id=cc_id
    )
    if success:
        flash('Centro de custo associado com sucesso.', 'success')
    else:
        flash(error, 'error')
    return redirect(url_for('financial.cost_centers', project_id=project_id))
...
def dissociate_cost_center(project_id: int, cc_id: int):
    """Dissociate a cost center from a project."""
    project = _get_project_or_abort(project_id)
    if project is None:
        return redirect(url_for('projects.index'))

    success, error = CostCenterService.dissociate_cost_center_from_project(
        project_id=project_id,
        cost_center_id=cc_id
    )
    if success:
        flash('Centro de custo desassociado com sucesso.', 'success')
    else:
        flash(error, 'error')
    return redirect(url_for('financial.cost_centers', project_id=project_id))
...
def toggle_cost_center_status(project_id: int, cc_id: int):
    """Toggle cost center status between active and blocked."""
    project = _get_project_or_abort(project_id)
    if project is None:
        return redirect(url_for('projects.index'))
    ...
    cc.status = 'blocked' if cc.status == 'active' else 'active'
    db.session.commit()
    
    status_label = 'bloqueado' if cc.status == 'blocked' else 'ativado'
    flash(f'Centro de custo {status_label} com sucesso.', 'success')
    return redirect(url_for('financial.cost_centers', project_id=project_id))
...
def delete_cost_center(project_id: int, cc_id: int):
    """Delete a cost center completely."""
    project = _get_project_or_abort(project_id)
    if project is None:
        return redirect(url_for('projects.index'))
    ...
    if success:
        flash('Centro de custo removido permanentemente.', 'success')
    else:
        flash(error, 'error')
    return redirect(url_for('financial.cost_centers', project_id=project_id))
